...
class raw_env(AECEnv):
        ...
        if self.render_mode == "human":
            ...
            for y in range(self.grid_size):
                for x in range(self.grid_size):
                    ...
                    if self.grid_for_display[PREDATOR, y, x] == 1:
                        pygame.draw.rect(self.render_window, COLORS['pred'], 
                                        (x * self.CELL_SIZE, y * self.CELL_SIZE, self.CELL_SIZE/2, self.CELL_SIZE))
                    if self.grid_for_display[PREY, y, x] == 1:
                        pygame.draw.rect(self.render_window, COLORS['prey'],
                                        (x * self.CELL_SIZE, y * self.CELL_SIZE, self.CELL_SIZE, self.CELL_SIZE/2))
                        
            
                    #If SELF is true, small green circle
                    if self.grid_for_display[SELF, y, x] == 1:
                        pygame.draw.circle(self.render_window, COLORS['self'], 
                                        (x * self.CELL_SIZE + self.CELL_SIZE//2, y * self.CELL_SIZE + self.CELL_SIZE//2), self.CELL_SIZE//4)
    ...
    def generate_grid(self,grid_size):
        ...
        #spawn the predator and prey in the corners
        grid[PREDATOR,1,1] = 1
        grid[PREY,grid_size-2,grid_size-2] = 1
        grid[PREDATOR,2,2] = 1
        grid[PREY,grid_size-2,1] = 1

        self.infos["pred_1"]["coords"] = [1,1]
        self.infos["pred_2"]["coords"] = [2,2]
        self.infos["hider_1"]["coords"] = [grid_size-2,grid_size-2]
        self.infos["hider_2"]["coords"] = [grid_size-2,1]
        return grid
    
    def common_grid_to_agent_observation(self,grid,agent):
        agent_observation = grid.copy()
        #get agent coords
        coords = self.infos[agent]["coords"]
        #make sure to wipe out agent observation[SELF]
        agent_observation[SELF] = 0
        agent_observation[SELF,coords[0],coords[1]] = 1
    ...
    def reset(self, seed=None, options=None):
        ...
        self.state = {agent: self.common_grid_to_agent_observation(self.grid_for_display,agent) for agent in self.agents} #TODO for now state and obs are equal
        self.observations = {agent: self.common_grid_to_agent_observation(self.grid_for_display,agent) for agent in self.agents} #TODO for now state and obs are equal
        self.num_moves = 0
        """
    ...
    def get_step_after_action(self,grid_for_agent,action):
        ...
        #assert the SELF is 1 in the grid

        assert grid_for_agent[SELF,self.infos[self.agent_selection]["coords"][0],self.infos[self.agent_selection]["coords"][1]] == 1
    ...
    def write_grid_from_info(self,grid_to_change,info,agent_self):
        self_coords = info[agent_self]["coords"]
        grid_to_change[SELF] = 0
        grid_to_change[SELF,self_coords[0],self_coords[1]] = 1

        #write zeros to grid_to_change[PRED]
        grid_to_change[PREDATOR] = 0
        grid_to_change[PREY] = 0

        for agent in self.agents:
            if agent.startswith("pred"):
                coords = info[agent]["coords"]
                grid_to_change[PREDATOR,coords[0],coords[1]] = 1
            else:
                coords = info[agent]["coords"]
                grid_to_change[PREY,coords[0],coords[1]] = 1
    ...
    def compute_rewards_all_agents(self):
        ...
        for agent in self.agents:
            if agent.startswith("pred_1"):
                #rewards[agent] = -np.abs(target_x-p1x) - np.abs(target_y-p1y)
                rewards[agent] = p1x
