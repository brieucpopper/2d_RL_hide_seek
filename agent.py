...
from networks import Critic, Actor,CriticCnn,ActorCnn, ActorCnnAlex, CriticCnnAlex
import numpy as np
import math
import copy
...
class CQLSAC(nn.Module):
    ...
    def __init__(self,
        ...
        self.actor_local = ActorCnn(self.env_size, self.num_actions).to(device)
        #self.actor_local = ActorCnnAlex(self.env_size, self.num_actions).to(device)
        ...
        self.critic1 = CriticCnn(self.env_size, self.num_actions).to(device)
        self.critic2 = CriticCnn(self.env_size, self.num_actions).to(device)
        #self.critic1 = CriticCnnAlex(self.env_size, self.num_actions).to(device)
        #self.critic2 = CriticCnnAlex(self.env_size, self.num_actions).to(device)
        
        assert self.critic1.parameters() != self.critic2.parameters()
        
        #self.critic1_target = Critic(state_size, action_size, hidden_size).to(device)
        self.critic1_target = CriticCnn(self.env_size, self.num_actions).to(device)
        #self.critic1_target = CriticCnnAlex(self.env_size, self.num_actions).to(device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = CriticCnn(self.env_size, self.num_actions).to(device)
        #self.critic2_target = Critic(state_size, action_size, hidden_size).to(device)
        #self.critic2_target = CriticCnnAlex(self.env_size, self.num_actions).to(device)
